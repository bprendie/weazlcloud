...
This does not complete the design above or authorize production enablement. D3 content-defined chunk objects, encrypted manifests, and the Restic comparison are implemented and recorded above. D4.5 now includes destination/index allocation accounting and uses the shared-write workspace reservation for migration; its expanded replacement/Trash/migration acceptance matrix remains open. The offline D5 tool is implemented but has not been rehearsed on a populated disposable volume. Legacy retirement, rollback, and production migration remain disabled.
