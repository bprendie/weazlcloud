...
Status: D0–D3 complete. D4.5 accounting implementation is in place but its lifecycle matrix remains open. D5 forward migration, recovery, and verification acceptance pass on disposable data; source retirement remains gated on D6. Shared storage and migration remain opt-in.
...
The mixed-backend integration is implemented behind `WEAZLCLOUD_STORAGE_BACKEND=shared-experimental`; the default remains `restic`. It includes durable per-upload backend/operation identity, mixed reads and owner-bound copy grants, archive holds, shared Trash/account cleanup, startup and idle reconciliation, claimed-object recovery, shared aggregate metrics, and conservative source-plus-destination upload reservations. D4.5 now reports destination, object, manifest, and index space separately and applies shared-write workspace reservations to migration. Its expanded acceptance matrix remains to be recorded. Do not enable this setting for production users before D6 and D7.
...
- [ ] **D4.5 — Integrate quota and useful measurements.** Implemented conservative reservations for source/destination coexistence, staging, encryption overhead, SQLite/index growth, and worst-case chunk manifests; resumable uploads and offline migration use the same shared-write reservation formula. Shared metrics separate logical bytes, unique plaintext chunks, allocated objects/manifests, SQLite index files, and temporary staging. The dedupe scope is explicitly shared-backed live and retained-Trash references; the host quota reports actual volume usage across both storage backends. **Acceptance evidence still needed:** duplicate/unique writes, replacement, Trash, migration, concurrent uploads, and reservation reconstruction must produce consistent figures. No per-user allowance or visible reserve percentage appears.
...
Phase exit: D3 complete; D4.5 implementation is in place, with the expanded lifecycle acceptance matrix still open. Earlier `make check`, authenticated browser smoke, and disposable-container smoke passed in legacy and mixed modes; those checks predate the latest D4.5 changes. Keep shared storage experimental and disabled for production until D4.5, migration, rollback, and rehearsal gates pass. Other G5 cleanup jobs remain separate.

## D5 — Build the migration engine

The initial D5 implementation is an offline, serial operator command, not an idle worker. It checks that the service ports are stopped, takes an exclusive data-directory lock, and stores per-entry progress in the shared index. Live and under-30-day Trash entries are copied and switched one at a time. Restart scans the encrypted catalog and reconciles shared operations before continuing. The library remains readable in mixed mode after service restart.
...
- [x] **D5.1 — Build a dry-run inventory.** `weazlcloud -migrate dry-run` and `status` use read-only account/catalog loading. Reports include live/Trash ages, shared and remaining files, staging and resumable uploads, distinct Restic snapshots, allocated legacy bytes, blockers, a no-dedupe destination bound, and peak workspace without private paths. Root-level legacy catalogs block start/resume before they create migration state. **Evidence:** `TestMigrationRunResumesVerifiesAndKeepsResticSources` creates a shared Restic batch snapshot and same-path live/Trash versions, verifies expired Trash, flags staged and resumable uploads, checks path privacy, and compares every data-directory file hash/mode/mtime before and after dry-run. `TestDryRunReportsUnreadableAccountWithoutWriting` covers an unreadable account and confirms no writes.
- [x] **D5.2 — Stream a pinned version to shared storage.** The copier holds the exact legacy reference, unlocks with the stored node key, reserves the shared-write quota bound, validates source size/hash, authenticates the destination readback, then switches. **Evidence:** the populated Restic migration test verifies empty and nonempty files plus Trash, reopens the shared backend, hashes every destination, and confirms the original Restic snapshot set is unchanged. `TestMigrationLeavesLegacyReadableOnQuotaAndSourceFailures` injects insufficient quota and corrupt source bytes; neither changes the catalog or makes the original readable source unavailable.
- [x] **D5.3 — Switch one catalog version atomically.** The expected-ID/revision switch preserves the path, metadata, live/Trash state, and deletion timestamp, and does not modify a Restic snapshot. **Evidence:** `TestMigrationRejectsChangedCatalogVersions` changes an inventoried item by rename, restore, overwrite, and delete before the switch; every stale switch is rejected and the resulting source/catalog state remains readable.
- [x] **D5.4 — Resume, pause, and reconcile.** Schema-6 per-entry states, an exclusive process lock, a persistent pause marker, catalog-authoritative recovery, and aborted-operation retry are implemented. **Evidence:** the operator test pauses, refuses to process staged uploads, resumes, repeats resume idempotently, and checks journal status. `TestMigrationReconcilesProcessStopsAtEveryDurableStage` terminates a child process at discovered, source-pinned, copied, destination-verified, and catalog-switched; every restart either keeps the Restic version readable or completes the switched shared version, and a retry converges to one verified version.
- [x] **D5.5 — Implement an explicit storage mode and operator controls.** `weazlcloud -migrate ACTION` provides `dry-run`, `status`, `start`, `pause`, `resume`, and `verify`; the existing `WEAZLCLOUD_STORAGE_BACKEND` setting selects the service mode. Status includes cumulative completed/failed counts, verified bytes, and fixed safe failure categories. **Evidence:** the disposable-volume migration test exercises each action, verifies remaining files cause `verify` to fail, checks repeated resume, and confirms `retire` refuses to run before D6. `TestMigrationJournalGroupsSafeFailureCategories` rejects user paths as status categories.
- [ ] **D5.6 — Retire legacy data by reference, not by owner completion percentage — BLOCKED BY D6.** `-migrate retire` remains disabled until rollback, reverse migration, and recovery acceptance pass. Keep every Restic source repository intact.

Phase exit: forward migration through full verification is implemented and passes the populated disposable Restic-volume rehearsal, including hard stops at every durable migration state. Source retirement stays gated by D6; production data has not been migrated.
