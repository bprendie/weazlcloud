...
Status: D0–D2 complete. D3 chunking/benchmarks are incomplete. D4 is integrated experimentally; D4.5 quota/workspace metrics remain open. Shared storage and migration remain disabled for production.
...
**D0–D2 are complete; D3 and the remaining D4.5 gate are outstanding. Do not merge Restic repositories or migrate real data.** The shared whole-file prototype proves basic ownership and recovery, but chunk dedupe and comparison are required before production adoption so we do not lose Restic's existing savings on similar disk images.
...
The mixed-backend integration is implemented behind `WEAZLCLOUD_STORAGE_BACKEND=shared-experimental`; the default remains `restic`. It includes durable per-upload backend/operation identity, mixed reads and owner-bound copy grants, archive holds, shared Trash/account cleanup, startup and idle reconciliation, claimed-object recovery, shared aggregate metrics, and conservative source-plus-destination upload reservations. D4 evidence now includes both browser and Docker smoke in both storage modes, Docker restart with resumable finalize, and shared-owner deletion/collector recovery tests. This remains an experimental whole-file store: D3 chunking and benchmarks remain incomplete, and D4.5 remains open. Do not enable this setting for production users.
...
Phase exit: pending D4.5 and D3. `make check`, authenticated browser smoke, and disposable-container smoke pass in legacy and mixed modes, including WebDAV and restart checks. D4.5 cannot pass on the whole-file prototype: chunk-scoped savings and chunk-manifest/workspace accounting depend on D3. Keep shared storage experimental and disabled for production until D3, D4.5, migration, rollback, and rehearsal gates pass. Other G5 cleanup jobs remain separate.
