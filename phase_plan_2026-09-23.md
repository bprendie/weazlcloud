...
Status: D0–D3 complete. D4.5 accounting and the D5 offline migrator are implemented; phase acceptance evidence and production rehearsal remain. D5 source retirement is disabled pending D6 rollback/recovery. Shared storage and migration remain opt-in.
...
- [ ] **D5.1 — Build a dry-run inventory.** `weazlcloud -migrate dry-run` and `status` are implemented. Read-only account/catalog loading avoids persisting compatibility upgrades. Reports count live files, folders, Trash ages, shared/eligible files, staged records, resumable upload records, distinct Restic snapshots, allocated legacy bytes, blockers, an output-size upper bound without dedupe savings, and peak per-file workspace, without private paths. It flags a catalog left at the legacy root. Disabled accounts are inventoried and skipped; deleting accounts are ignored. **Acceptance still needed:** verify batch snapshots, same-path live/Trash, unreadable accounts, staging, pending uploads, and a byte-for-byte no-write dry-run.
- [ ] **D5.2 — Stream a pinned version to shared storage.** Implemented serial streaming from the exact Restic reference, with an in-process source hold, owner node-key unlock, global quota reservation, metadata validation, full source hash/length check, and authenticated destination readback before switching. **Acceptance still needed:** inject source corruption and insufficient-space failures and confirm the legacy reference stays readable.
- [ ] **D5.3 — Switch one catalog version atomically.** Implemented an expected-ID/revision switch that preserves path, metadata, live/Trash state, and deletion timestamp. It changes no Restic snapshot and rejects stale entries. The first version requires services to be offline, preventing concurrent account deletion or edits during a switch. **Acceptance still needed:** exercise rename, restore, overwrite, and deletion between inventory and switch.
- [ ] **D5.4 — Resume, pause, and reconcile.** Implemented schema-6 per-entry states, an exclusive process lock, pause marker, catalog-authoritative restart reconciliation, and safe repeat runs. **Acceptance still needed:** stop at each durable stage, restart, and verify convergence.
- [ ] **D5.5 — Implement an explicit storage mode and operator controls.** Implemented `dry-run`, `status`, `start`, `pause`, `resume`, and `verify` actions via `weazlcloud -migrate ACTION`. The existing `WEAZLCLOUD_STORAGE_BACKEND` setting selects the mode when the service starts. Status reports counts and safe failure categories. **Acceptance still needed:** rehearse every action on disposable data. The destructive retirement action refuses to run.
- [ ] **D5.6 — Retire legacy data by reference, not by owner completion percentage.** Not implemented. `-migrate retire` is deliberately blocked until D6 reverse migration and recovery gates pass. Keep every Restic source repository intact.

Phase exit: migration code exists but D5 acceptance smoke and the populated disposable-volume rehearsal remain. No production data migration has run.
