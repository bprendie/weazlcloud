...
function admin() {
  ...
  return head('NODE ADMIN / ACCESS', 'Approve local accounts.', 'Identity approval only. User vaults and their contents stay outside the administrator view.') +
    ...
    `<h3>Local accounts</h3><p>Disable cuts off new sessions and grab links. Bytes already delivered during an active transfer cannot be recalled. Delete permanently removes that account’s data.</p><div class="file-list admin-requests">${state.adminUsers.length ? state.adminUsers.map(u => `<div class="file-row"><span class="kind ${u.deleting ? 'type-dir' : ''}">${u.deleting ? 'DELETING' : u.disabled ? (u.disable_pending ? 'REVOKING' : 'DISABLED') : 'ACTIVE'}</span><span><strong>${esc(u.full_name || u.username)}</strong><small>${esc(u.username)}${u.admin ? ' · administrator' : ''}${u.disable_error ? ` · disable cleanup pending: ${esc(u.disable_error)}` : ''}${u.delete_error ? ` · deletion pending: ${esc(u.delete_error)}` : ''}</small></span>${!u.deleting ? `<button class="text-button" data-user-toggle="${esc(u.id)}" data-disabled="${u.disabled && !u.disable_pending && !u.disable_error ? 'false' : 'true'}">${u.disabled ? (u.disable_pending || u.disable_error ? 'Retry revoke' : 'Enable') : 'Disable'}</button><button class="text-button" data-user-delete="${esc(u.id)}" data-username="${esc(u.username)}">Delete</button>` : '<small>Deletion will resume automatically.</small>'}</div>`).join('') : '<p class="empty">No accounts.</p>'}</div>` +
    `<h3 style="margin-top:28px">Access requests</h3><div class="file-list admin-requests">${state.requests.length ? state.requests.map(q => `<div class="file-row"><span class="kind ${q.status === 'pending' ? 'type-dir' : ''}">${esc(q.status.toUpperCase())}</span><span><strong>${esc(q.username)}</strong><small>${esc(q.note || 'No note')} · ${esc(new Date(q.created_at).toLocaleString())}</small></span>${q.status === 'pending' ? `<button class="text-button" data-admin-approve="${esc(q.id)}">Approve</button><button class="text-button" data-admin-reject="${esc(q.id)}">Reject</button>` : ''}</div>`).join('') : '<p class="empty">No account requests.</p>'}</div>`;
}
...
export function renderUploadTray() {
  ...
  const archiveJobs = state.archiveJobs || [];
  const hasUpload = upload && (!upload.dismissed && (upload.active || upload.total));
  if (!tray || (!hasUpload && !archiveJobs.length)) { if (tray) tray.hidden = true; return; }
  tray.hidden = false;
  const archiveMarkup = archiveJobs.slice(0, 3).map(job => `<div class="archive-job"><div><strong>ZIP DOWNLOAD</strong><span>${esc(job.status || 'queued')}</span></div><p>${job.status === 'ready' ? `${job.files} files ready` : job.status === 'failed' ? esc(job.error || 'failed') : job.status === 'cancelled' ? 'Cancelled' : 'Preparing on the node…'}</p>${job.status === 'ready' ? `<button class="text-button" data-archive-download="${esc(job.id)}">Download ZIP</button>` : job.status === 'queued' || job.status === 'preparing' ? `<button class="text-button" data-archive-cancel="${esc(job.id)}">Cancel</button>` : ''}</div>`).join('');
  if (!hasUpload) { tray.innerHTML = `<div class="upload-tray-head"><div><span class="eyebrow purple">LIBRARY / DOWNLOAD</span><strong>Background ZIP jobs</strong></div></div>${archiveMarkup}`; return; }
  const failed = upload.items?.filter(item => item.status === 'failed') || [];
  const needsFile = upload.items?.filter(item => item.status === 'needs-file') || [];
  const title = needsFile.length ? `${needsFile.length} upload${needsFile.length === 1 ? '' : 's'} waiting for file selection` : upload.active ? `Uploading · ${upload.done} of ${upload.total} complete` : `${upload.done} of ${upload.total} uploaded`;
  ...
  const pending = [...failed, ...needsFile];
  const errors = pending.length ? `<div class="upload-errors">${pending.slice(0, 4).map(item => `<div><strong>${esc(item.target)}</strong><span>${esc(item.error || 'Upload failed')}</span></div>`).join('')}${pending.length > 4 ? `<small>…and ${pending.length - 4} more</small>` : ''}</div>` : '';
  const controls = `<div class="upload-controls">${failed.length ? '<button class="text-button" data-upload-retry>Retry failed</button>' : ''}${needsFile.length ? '<button class="text-button" data-upload-reselect>Select files to resume</button>' : ''}${upload.active ? '<button class="text-button" data-upload-cancel>Cancel</button>' : '<button class="text-button" data-dismiss-upload>Dismiss</button>'}<button class="text-button" data-upload-collapse>${upload.collapsed ? 'Show details' : 'Collapse'}</button></div>`;
  tray.innerHTML = `<div class="upload-tray-head"><div><span class="eyebrow purple">LIBRARY / UPLOAD · 3 RAILS</span><strong>${title}</strong></div>${controls}</div>${upload.collapsed ? '' : `<div class="upload-rails">${rails}</div>${errors}`}${archiveMarkup}<progress max="100" value="${Math.min(100, upload.percent || 0)}"></progress><p class="eyebrow">${Math.round(upload.percent || 0)}% of ${formatBytes(upload.totalBytes || 0)} · ${failed.length ? `${failed.length} failed` : upload.active ? 'working in the background' : 'ready'}</p>`;
}
