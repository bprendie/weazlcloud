...
Status: G1.1–G1.4, G3.1–G3.2, G4.1–G4.3, and G5.1–G5.3 complete. D1 and D2 are resolved.
...
- [x] **G1.3 — Integrate space and lifecycle checks.** Reconstruct reservations after restart without counting already occupied bytes twice. Reserve remaining workspace, enforce actual disk headroom, release on cancel/24-hour expiry/commit, and reject writes for disabled/deleted owners. Do not clear usable sessions merely because the process restarted. Pass: reservation restoration/release, expiry cleanup, and shared-volume quota boundary tests pass; authenticated upload routes use the current active user.
- [x] **G1.4 — Resume from the browser.** Persist nonsecret queue metadata, reconcile it with owner-scoped server sessions after login, and preserve the three upload rails. Show server-confirmed progress separately from saving. Retry interrupted chunks with bounded backoff and reconcile the server-confirmed offset before retrying. Match reselected files using chunk content verification, not filename alone. Reset in-memory queue state on account change. Pass: restart mid-upload resumes from the confirmed offset and server list is owner-scoped; selecting the original file/folder again after reopening is the expected recovery flow. No local helper.
...
- The browser persists nonsecret session metadata and reconciles it with an owner-scoped server session list after login. After a browser reopen it asks the user to reselect the original file and verifies every server-confirmed chunk hash before sending only missing bytes. Interrupted chunks retry with bounded backoff; a lost response is reconciled against the server offset.
- Incomplete sessions expire after 24 hours; an application-owned sweep runs every 15 minutes and also removes expired sessions encountered during recovery/listing. Disk reservations are reconstructed from live manifests and released on cancel, expiry, or successful commit. Separate session locks preserve the three concurrent upload rails.
- Focused tests cover wrong offsets and hashes, interrupted chunks, restart recovery, repeated finalize, owner-scoped listing, reservation reconstruction/release, expiry cleanup, and stored-payload hashes. `make check`, browser smoke, recovery smoke, disposable-container smoke, and local test-container readiness passed. No production load test or large-payload peak-memory measurement was run; D10 keeps those out of scope for this workstation pass.
...
- [x] **G5.1 — Add a bounded idle coordinator.** Track active storage operations across Desk, WebDAV, uploads, previews, ZIPs, and grabs. Long-lived SSE connections and health probes must not permanently prevent idle. Make the quiet interval configurable and document its initial technical default. Run one maintenance job at a time; yield/cancel safely when foreground activity resumes. Pass: a fake-clock test covers idle eligibility, continuous probes/SSE, resumed activity, shutdown, and no overlapping jobs.
- [x] **G5.2 — Register cleanup jobs.** Include expired capsule payloads, expired/cancelled on-demand ZIPs, safe abandoned staging, bounded preview eviction, G3 Trash cleanup, and incomplete uploads after 24 hours. Keep the 90-minute ZIP policy; do not evict early for low space. Grab payloads follow their burn/revoke/expiry lifecycle, not ZIP expiry. Use retry/backoff and restart reconciliation; never delete active temp packs just because their names resemble stale files. Pass: jobs run without visiting a listing, active work survives, and interruption does not leak reservations or falsely mark cleanup complete.
- [x] **G5.3 — Report useful status.** Record last attempt/success, duration, reclaimed bytes where measured, and safe failure category. Avoid filenames and capability IDs. Pass: an induced disk/permission failure is distinguishable from an empty cleanup pass. Notification destination waits for D5.
...
September 22: Completed G3.1 safe-reclamation audit and fixes in commit `f0f247d`. Tests cover cutoff boundaries, empty folders, zero-byte files, replacement at a trashed path, and live/newer snapshot references. `make check` passed. G3.2 remains queued behind G5.1's idle coordinator and needs durable cleanup intent plus measured physical reclamation reporting.
September 22: Completed G5.1 and G3.2 in the current worktree. The coordinator tracks foreground HTTP storage requests plus queued upload commits and full ZIP jobs; health and library SSE are excluded. It runs one job at a time after a configurable five-minute quiet interval, cancels for resumed work, and waits for the interrupted job to release storage before foreground access proceeds. Trash maintenance uses the stored node key only when the user's vault was locked and restores that locked state afterward. Encrypted retry intent survives catalog-purge/Restic-prune failures; expired Trash is processed on idle without opening the Trash page. The retry test injected one prune failure and then verified successful retry and positive allocated repository-byte reclaim. `make check`, recovery smoke, authenticated browser smoke, and fresh disposable-container smoke passed. Rebuilt `weazlcloud-local-test` as `weazlcloud:local-test-g3`, preserved volume `weazlcloud-local-test-data-20260922`, and confirmed `/ready`; no production host was touched. G5.2's remaining cleanup jobs and G5.3 status reporting are still open.
September 22: Added an end-to-end filesvc test for an eligible fake-clock idle pass against a disposable Restic repository. It ages one Trash item past 30 days and leaves another at 29 days, verifies cleanup relocks the vault, restores and reads the newer item, and confirms only its Restic snapshot remains. Focused storage tests and `make check` passed; no production data was touched. G3.1 and G3.2 acceptance evidence is now directly covered, including newer-Trash restore after automatic cleanup.
September 23: Completed G5.2/G5.3. Registered idle passes for pending account cleanup, expired grab payloads, on-demand ZIPs, staging recovery, bounded preview caches, 30-day Trash, and 24-hour incomplete uploads. ZIP payloads expire 90 minutes after they become ready; restart cleanup uses archive-file modification time, while active worker temp files are protected by activity tracking. Upload expiry no longer deletes stale sessions during startup reconciliation; reservations and files are released together during the idle pass. Persisted admin-only status reports last attempt/success, duration, reclaimed bytes, and safe error categories without user filenames or capability IDs. Retries occur on the next quiet interval (five-minute default). D5 notification destination remains open; there is no email or external notification integration. `make check` passed, including race tests and line limits. Fresh `weazlcloud:g5-smoke` image and disposable container smoke passed readiness, login/unlock, admin maintenance status, upload/download, preview, and grab. No production system or data touched.
