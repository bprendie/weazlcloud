...
- [x] **P4.2 — Bound preview work.** Limit simultaneous render jobs and total cache size; prioritize visible cards and cancel obsolete requests. Limit parser memory, archive expansion, and execution time without restricting upload size. Pass: a large folder or malformed preview file does not monopolize uploads or exhaust the node.
...
Phase exit: complete for the preview implementation. List/grid views share preview capabilities; generic MIME labels do not unnecessarily prevent previews; supported audio/video play inline; visible preview work is bounded and upload warming is low priority. The renderer baseline and authenticated browser smoke passed. A full production-volume browser paint measurement remains an operational follow-up because standalone REST uploads create one restic commit per file and are not a useful preview benchmark.
...
- [x] **P5.2 — Add multi-select and batch actions.** Implement Ctrl/Shift selection and bulk move, download, and delete with clear partial-failure reporting. Keep keyboard and context-menu actions consistent. Pass: a failed item does not hide successful items or clear an unrelated selection.
...
Date: September 21, 2026
Task ID: P4.5
Commit: 11e1116, c1766e6
Changed: Added an authenticated per-file capability endpoint that combines filename, declared MIME, and a bounded content prefix. The grid now requests capabilities for visible cards before choosing thumbnail, text, model, PDF, media, or download rendering. Generic and extensionless content gets a deliberate fallback; a PNG stored as `mislabeled.txt` is classified as a thumbnail and successfully renders through the thumbnail endpoint. SVG grid and modal previews are now escaped text, and known media/model/PDF cards avoid capability replacement that could disrupt native controls.
...
Known limitations / decisions needed: Media artwork/poster generation, broader media capability fixtures, and full browser playback coverage remain open. The 2 MiB content-inspection threshold intentionally avoids restoring large generic binaries merely to identify them; those fall back to download unless the filename identifies a supported format.
...
Known limitations / decisions needed: Video poster extraction and audio artwork cards remain renderer follow-ups; native controls, expanded playback, unsupported-codec fallback, and the browser play/seek/refresh gate are covered by the later P4.6 work log.
Next task: P4.2/P4.7 scheduling and P4.8 warming.

Date: September 21, 2026
Task ID: P4.2, P4.6–P4.9
Commit: f9e466c, pending
Changed: Added active cleanup for detached text, capability, and thumbnail requests; kept four thumbnail, three text, and two capability rails; refreshed cache access times for bounded eviction; added low-priority thumbnail warming after successful browser uploads and cancelled it on vault lock. Added native media fallback cards, maintained single-player playback, and fixed the grid observer to cover nested cards. The authenticated node smoke verified capability detection, private thumbnails, media byte ranges, browser play/seek/refresh behavior, and upload-triggered warm requests.
...
Task ID: P5.2
Commit: 7d814bc
Changed: Added Ctrl/Cmd and Shift multi-select, selection-aware toolbar and whitespace context menus, bulk move/delete, and partial-failure reporting while preserving failed selections.
Checks run and results: `go test ./...` passed; `go vet ./...` passed; targeted `go test -race ./internal/filesvc ./internal/library` passed; JavaScript syntax checks and `git diff --check` passed; Chromium selection smoke verified two selected files, four batch controls, and no reload.
Known limitations / decisions needed: Backend bulk ZIP work was the next dependent task.
Next task: P5.2a–P5.2c server-side ZIP jobs.

Date: September 21, 2026
Task ID: P5.2a–P5.2c implementation
Commit: 9cb1407
Changed: Added owner-scoped background ZIP jobs with immutable restic manifests, nested and empty-directory preservation, Unicode-safe relative paths, deterministic overlap resolution, two-worker streaming, shared quota reservations, data-volume staging, transfer-tray status, cancellation, vault-lock cleanup, restart temp reconciliation, and authenticated range downloads. Folder context menus now use the same archive path; single-file downloads remain direct.
Checks run and results: `go test ./...` passed; `go vet ./...` passed; `go test -race ./internal/filesvc ./internal/library` passed; JavaScript syntax checks passed; `make desk-assets` and `git diff --check` passed. Localhost smoke created a mixed archive containing an empty directory and a 1 MiB binary, verified ZIP contents and Unicode-safe paths, replaced a source after manifest capture without changing the archive, received `206` for a byte range, and received `401 vault is locked` after locking.
Known limitations / decisions needed: The live smoke used temporary data under `/tmp`; the archive implementation stages under each user's configured data directory. On-demand ZIP retention is 90 minutes. Folder/file grab capsules retain their encrypted payload until their existing burn/revoke terminal state.
Next task: P5.3 deletion and retention.
...
Next task: Phase 6 release and recovery work.
