...
docker run --rm --memory=2g -v "$volume:/data" -v "$PWD:/src:ro" -w /src \
  ...
  golang:1.25-bookworm bash -c '
    ...
    CGO_ENABLED=0 go test ./internal/sharedstore -count=1 -v | tee /data/sharedstore-results.txt
    CGO_ENABLED=0 go test ./internal/library -run "^TestSharedBackendMixedLibraryLifecycle$" -count=1 -v | tee /data/library-results.txt
  '
